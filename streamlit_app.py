...
input_name = st.text_input(label="In Game Name", value=None)
if input_name:
    if st.session_state.player_data["player_name"] != input_name:
        st.session_state.player_data["player_name"] = input_name
        # get from wiseoldman
        try:
            player_data = (
            ...
            if player_data:
                st.session_state.player_data["player_fletch_xp"] = (
                    player_data.get("skills").get("fletching").get("experience")
                )
                st.session_state.player_data["player_construction_xp"] = (
                    player_data.get("skills").get("construction").get("experience")
                )
                st.session_state.player_data["player_woodcutting_lvl"] = (
...
with input_col1:
    curr_fletching_xp = st.number_input(
        ...
        min_value=0,
        value=(st.session_state.player_data["player_fletch_xp"]),
...
with input_col2:
    curr_woodcutting_lvl = st.number_input(
        ...
        min_value=1,
        max_value=99,
        value=st.session_state.player_data["player_woodcutting_lvl"],
    )
with input_col3:
    curr_con_xp = st.number_input(
        label="Current Construction XP",
        min_value=0,
        value=st.session_state.player_data["player_construction_xp"],
        max_value=level_to_xp(99),
    )
